...
export default function ChatCitationsPanel({
  ...
  return (
    <aside className="flex w-[272px] shrink-0 flex-col rounded-[20px] border border-slate-200/80 bg-white p-3 shadow-[0_10px_26px_rgba(15,23,42,0.04)]">
      ...
      <div className="mt-3 flex-1 space-y-2 overflow-y-auto pr-1">
        {citations.length === 0 ? (
          ...
          citations.map((citation, index) => {
            ...
            return (
              <div
                key={`${citation.chunkIndex}-${citation.pageStart}-${citation.pageEnd}-${index}`}
      ...
      <button
        type="button"
        className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-[13px] font-medium text-blue-600 transition hover:bg-slate-100"
      >
        Show less
      </button>
    </aside>
